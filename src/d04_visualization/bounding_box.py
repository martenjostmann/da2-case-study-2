    ...
    for y_i, y in enumerate(pred):
        for x_i, x in enumerate(y):
            if x != 0:
                ...
                rect = patches.Rectangle(corner, 256, 256, linewidth=0.5, edgecolor=colors[int(x) - 1],
                                         facecolor='none')
...
def draw_boxes(image, boxes, stride, save=False):
    ...
    stride: int
      Step size which was used for the sliding window approach
    save: bool
      Should the resulting image be saved to a file (default False)
    ...
    for i, (clazz, x, y) in enumerate(boxes):
        rect = patches.Rectangle((x * stride, y * stride), 256, 256, linewidth=0.5, edgecolor=colors[int(clazz) - 1],
                                 facecolor='none')
